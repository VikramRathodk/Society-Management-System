...
- ⏳ **Next:** `application`/`interfaces` layers are still empty for `user` and `society` — no services or controllers yet. Then auth (password hashing, `POST /auth/login`, `JwtAuthFilter`), then wiring `PermissionService` + `@RequiresPermission` onto one endpoint (`POST /users`) as a proof of concept. Note: Spring Boot 4 relocated test-slice classes into per-feature modules — `@DataJpaTest` is now `org.springframework.boot.data.jpa.test.autoconfigure.DataJpaTest`, `TestEntityManager` is `org.springframework.boot.jpa.test.autoconfigure.TestEntityManager`, `@AutoConfigureTestDatabase` is `org.springframework.boot.jdbc.test.autoconfigure.AutoConfigureTestDatabase` — not the old `org.springframework.boot.test.autoconfigure.*` paths from Boot 3.
